...
import numpy as np
import cv2
from packages.realsense_device_manager import post_process_depth_frame
from packages.helper_functions import get_corner_3d, get_clipped_pointcloud, get_clipped_pointcloud2, convert_depth_frame_to_pointcloud4, \
...
def corners_from_pc(pcloud, corner_direction):
	...
	if -hmin > 0.02:
		...
		#print ('---- small ------')
		h_delta = 0.006
		point_cloud = pcloud2[:, pcloud2[2, :] < hmin + h_delta]
...
def calculate_cumulative_pointcloud(frames_devices, calibration_info_devices, roi_2d, corner_direction, depth_threshold = 0.01):
	"""
 Calculate the cumulative pointcloud from the multiple devices
	...
	for (device, frame) in frames_devices.items() :
		...
		filtered_depth_frame = post_process_depth_frame(frame[rs.stream.depth], temporal_smooth_alpha=0.1, temporal_smooth_delta=40)
		depth_img = np.asarray(filtered_depth_frame.get_data())
		cv2.imwrite("depth_{}.png".format(device), depth_img)

		point_cloud = convert_depth_frame_to_pointcloud5(depth_img.copy(), calibration_info_devices[device][1][rs.stream.depth])
		...
		#-------------------------   get corner ----------------------
		corner = corners_from_pc(point_cloud, corner_direction[device])
		#-------------------------------------------------------------
	...
	"""point_cloud_cumulative = point_cloud_cumulative.astype(np.float32)
	cloud = pcl.PointCloud(point_cloud_cumulative.transpose())
	detector = cloud.make_HarrisKeypoint3D()
	detector.set_NonMaxSupression(True)
	detector.set_Radius(0.03)

	keypoints = detector.compute()

	corners = np.zeros((3, keypoints.size), dtype=np.float32)
	intensity = []
	for i in range(keypoints.size):
		corners[0][i] = keypoints[i][0]
		corners[1][i] = keypoints[i][1]
		corners[2][i] = keypoints[i][2]
		intensity.append(keypoints[i][3])"""
	...
	if point_cloud.shape[1] >= 4:
		...
		for (device, calibration_info) in calibration_info_devices.items():
			...
			for bounding_box_point in bounding_box_device_3d: 
				...
				infrared_pixel.append(rs.rs2_project_point_to_pixel(calibration_info[1][(rs.stream.infrared, 1)], bounding_box_point))
		...
		if (length != 0 and width != 0):
			...
			for i in range(len(corners)):
				...
				if x < 400:
					...
					px = x
					py = y + 50
				cv2.putText(infrared_image, pt_text, (px, py), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255))
			print ('---------')
		# Visualise the results
		cv2.imshow('Color image from RealSense Device Nr: ' + device, infrared_image)
		cv2.waitKey(1)